// ============================================================
// App.jsx — Main Orchestrator (Refactored)
// Now focuses on high-level layout and profile gating.
// State is managed via GamifyContext and ModuleDataContext.
// ============================================================
import { useState, useEffect, useCallback, Component } from 'react'
import { AlertTriangle } from 'lucide-react'
import { LanguageProvider } from './hooks/useLanguage'
import { useProfile } from './hooks/useProfile'
import { GamifyProvider, useGamify } from './contexts/GamifyContext'
...
import { FloatingAmanahOverlay } from './components/ui/FloatingAmanahOverlay'
...
function AppInner() {
  ...
  const [isAmanahOverlayOpen, setIsAmanahOverlayOpen] = useState(false)
  ...
  const handleAmanah = useCallback((id, isAmanah) => {
    ...
    if (!isAmanah && !current.hasSeenBias) {
      setIsAmanahOverlayOpen(true)
    }
  ...
  const coreModals = (
    <>
      ...
      <AcademicKnowledgeBase isOpen={isKnowledgeBaseOpen} onClose={() => setIsKnowledgeBaseOpen(false)} />
      <LevelUpOverlay notification={gamify.notification} onDismiss={gamify.dismiss} />
      <FloatingAmanahOverlay isOpen={isAmanahOverlayOpen} onClose={() => setIsAmanahOverlayOpen(false)} />
    </>
  )
        ...
        <div className="min-h-screen" style={{ background: isDark
          ...
          <div className="absolute top-4 right-4 flex gap-2 z-10">
            <button
              ...
              className="p-2 rounded-lg bg-white/60 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300"
            >
              {isDark ? '☀️' : '🌙'}
            </button>
          </div>
          <WelcomeScreen
            onSubmit={(data, freshStart) => {
              if (freshStart) {
                ...
                localStorage.removeItem('statslab_sus_data')
              }
              saveProfile(data)
            }}
      ...
      <div className="min-h-screen">
        ...
        >
          <div key={activeModule} className="animate-module-entry">
            ...
            {activeModule === 'certificate' && (
              <div className="space-y-5 animate-fade-in">
                <div className="flex items-center justify-between">
                  <h2 className="text-xl font-extrabold text-slate-800 dark:text-slate-100">🏅 Sertifikat Penghargaan</h2>
            ...
            {activeModule === 'sus' && <SUSForm profile={profile} onBack={() => setActiveModule('ziswaf')} />}
          </div>
        </MainLayout>

        <BottomNav activeModule={activeModule} onModuleChange={handleModuleChange} />
        <GuideBubble gamify={gamify} profile={profile} activeModule={['certificate', 'sus'].includes(activeModule) ? null : activeModule} />
        <Toast notification={gamify.notification} lang="id" onDismiss={gamify.dismiss} />
        
        {showResearcher && (
          <ResearcherPortal profile={profile} gamify={gamify} onExit={() => setShowResearcher(false)} />
        )}
...
class ErrorBoundary extends Component {
  ...
  componentDidCatch(error, errorInfo) { console.error('STATSLAB GLOBAL CRASH:', error, errorInfo) }
  render() {
      ...
      return (
        <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6 text-center">
          <div className="max-w-md space-y-6">
            ...
            <h1 className="text-3xl font-black text-white italic uppercase tracking-tighter">Ops! Terjadi Kesalahan</h1>
            <p className="text-slate-400 font-medium">Sistem mendeteksi adanya kegagalan runtime. Kita perlu menyegarkan aplikasi.</p>
            <button onClick={() => { localStorage.clear(); window.location.reload() }} className="w-full py-4 bg-emerald-600 hover:bg-emerald-500 text-white font-black rounded-2xl shadow-xl shadow-emerald-900/40 transition-all active:scale-95">RESET & MULAI ULANG (RESCUE)</button>
          </div>
        </div>
      )
    }
    ...
    if (this.state.hasError) {
      return (
        <div className="p-8 text-center glass-card border-2 border-rose-500/20 bg-rose-50/50 dark:bg-rose-900/10">
          ...
          <h3 className="text-xl font-black text-slate-800 dark:text-white uppercase tracking-tighter italic">Ups! Modul ini macet</h3>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-2 mb-6">Terjadi kegagalan saat merender data. Tenang saja, kamu bisa mereset data modul ini untuk melanjutkan.</p>
          <button onClick={() => { localStorage.removeItem('statslab_module_data'); window.location.reload() }} className="px-6 py-3 bg-rose-600 hover:bg-rose-500 text-white font-black rounded-xl transition-all shadow-lg active:scale-95">RESET DATA & REFRESH</button>
        </div>
      )
    }
    return this.props.children

    ...
    <aside className="hidden lg:flex flex-col w-72 bg-white/50 dark:bg-slate-950/20 border-r border-slate-200 dark:border-slate-800/50 backdrop-blur-xl h-[calc(100vh-4rem)] sticky top-16 no-print overflow-y-auto custom-scrollbar">
      ...
      <div className="p-6">
        ...
        <div className="glass-card p-5 border-2 border-slate-100 dark:border-slate-800/50 bg-white/40 dark:bg-slate-900/40">
          <div className="flex items-center justify-between mb-3">
            <span className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400">Peringkat Akademik</span>
          ...
          <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-4">Tahap: {currentRank.theory}</p>
      ...
      <div className="flex-1 px-4 pb-6 space-y-6">
        ...
        <nav className="space-y-1">
          <h4 className="px-3 text-[9px] font-black text-slate-400 uppercase tracking-[0.3em] mb-4">Workspace</h4>
            ...
            >
              <div className={`p-1.5 rounded-lg transition-colors ${activeModule === item.id ? 'bg-white/10' : 'bg-slate-100 dark:bg-slate-800 group-hover:bg-emerald-50'}`}>
                <PremiumIcon 
                  id={item.icon} 
                  size={16} 
                  className={activeModule === item.id ? 'text-white' : 'text-slate-400 group-hover:text-emerald-600'} 
                />
              </div>
        ...
        <div className="pt-6 border-t border-slate-100 dark:border-slate-800">
          <div className="flex items-center justify-between mb-4 px-3">
             <h4 className="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em]">Koleksi Gelar</h4>
        ...
        <div className="space-y-1">
          <button onClick={() => onModuleChange('certificate')} className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all ${activeModule === 'certificate' ? 'bg-indigo-50 text-indigo-700' : 'text-slate-500 hover:bg-slate-50'}`}>

...
export async function POST(req: Request) {
  try {
    ...
    const { answers, totalScore, adjectiveRating, sessionId } = body;
    ...
    // Link SUS to the existing session (if provided) instead of creating an orphan session.
    if (sessionId) {
    ...
    // Fallback: no valid session yet — create one (keeps data valid for R&D).

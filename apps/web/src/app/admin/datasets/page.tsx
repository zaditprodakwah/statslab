...
export default function AdminDatasetsPage() {
  ...
  const updateTask = (d: any, index: number, field: keyof TaskDraft, value: string | number) => {
    ...
    drafts[index] = {
      ...drafts[index],
      [field]: field === "taskNumber" || field === "watsonLevel" ? Number(value) : value,
    };
  ...
  const deleteDataset = async (d: any) => {
    if (!window.confirm(`Hapus dataset "${d.title}" beserta semua soalnya? Tindakan ini permanen.`))
      return;
    try {
      const res = await fetch(`/api/admin/datasets/${d.id}`, {
        method: "DELETE",
    ...
    <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "40px 20px" }}>
      ...
      {saveMsg && (
        <div
          style={{
            ...
            fontWeight: 600,
            fontSize: "0.9rem",
            backgroundColor: saveMsg.type === "ok" ? "var(--color-emerald-50)" : "#fee2e2",
            color: saveMsg.type === "ok" ? "var(--color-emerald-700)" : "#b91c1c",
          }}
        >
          {saveMsg.text}
        </div>
      ...
      <div className="glass-panel" style={{ padding: "24px", marginBottom: "32px" }}>
        ...
        <p style={{ fontSize: "0.85rem", color: "var(--text-secondary)", marginBottom: "12px" }}>
          Tempelkan skema JSON dataset lengkap (beserta array `tasks`) untuk menambah modul baru ke
          sistem.
        </p>
        ...
        {uploadMsg && (
          <div style={{ fontSize: "0.9rem", marginBottom: "12px", fontWeight: 600 }}>
            {uploadMsg}
          </div>
        )}
                    ...
                    <>
                      ...
                      >
                        ...
                        <span style={{ fontSize: "0.8rem", color: "var(--text-secondary)" }}>
                          Pilar Islam: <strong>{d.islamicValue}</strong> • {d.tasks?.length || 0}{" "}
                          Soal
                        </span>
                      </div>
                      <p
                          ...
                          >
                            {d.tasks.map((t: any) => (
                              <li key={t.id} style={{ marginBottom: "4px" }}>
                                <strong>Tugas #{t.taskNumber}</strong> (Watson L{t.watsonLevel},{" "}
                                {t.inputType}): {t.prompt}
                              </li>
                            ))}
                          </ul>
                        </div>
                    ...
                    <>
                      ...
                      >
                        ...
                        >
                          ...
                          <input
                            value={d.title}
                            onChange={(e) => updateMeta(d.id, "title", e.target.value)}
                            style={inputStyle}
                          />
                        </label>
                        <label
                        ...
                        >
                          ...
                          <input
                            value={d.category}
                            onChange={(e) => updateMeta(d.id, "category", e.target.value)}
                            style={inputStyle}
                          />
                        </label>
                        <label
                        ...
                        >
                          Pilar Islam
                          <input
                            value={d.islamicValue}
                            onChange={(e) => updateMeta(d.id, "islamicValue", e.target.value)}
                            style={inputStyle}
                          />
                        </label>
                      </div>
                      ...
                      >
                        ...
                        <textarea
                          rows={2}
                          value={d.description || ""}
                          onChange={(e) => updateMeta(d.id, "description", e.target.value)}
                        ...
                        {drafts.map((t, idx) => (
                          ...
                          >
                            <div
                              style={{
                                display: "grid",
                                gridTemplateColumns: "90px 110px 160px 1fr",
                                gap: "8px",
                                marginBottom: "8px",
                              }}
                            >
                              <label style={smallLabel}>
                                ...
                                <input
                                  ...
                                  min={1}
                                  value={t.taskNumber}
                                  onChange={(e) => updateTask(d, idx, "taskNumber", e.target.value)}
                                  style={inputStyle}
                                />
                              </label>
                              <label style={smallLabel}>
                                ...
                                <select
                                  value={t.watsonLevel}
                                  onChange={(e) =>
                                    updateTask(d, idx, "watsonLevel", e.target.value)
                                  }
                                  style={inputStyle}
                                >
                              ...
                              <label style={smallLabel}>
                                ...
                                <input
                                  value={t.indicator}
                                  onChange={(e) => updateTask(d, idx, "indicator", e.target.value)}
                                  style={inputStyle}
                                />
                              </label>
                              <label style={smallLabel}>
                                ...
                                <select
                                  value={t.inputType}
                                  onChange={(e) => updateTask(d, idx, "inputType", e.target.value)}
                                  style={inputStyle}
                                >
                            ...
                            <label style={smallLabel}>
                              ...
                              <textarea
                                rows={2}
                                value={t.prompt}
                                onChange={(e) => updateTask(d, idx, "prompt", e.target.value)}
                                style={{ ...inputStyle, fontFamily: "inherit" }}
                              />
                            </label>
                            <div
                            ...
                            >
                              <label style={smallLabel}>
                                ...
                                <textarea
                                  rows={1}
                                  value={t.clue}
                                  onChange={(e) => updateTask(d, idx, "clue", e.target.value)}
                                  style={{ ...inputStyle, fontFamily: "inherit" }}
                                />
                              </label>
                              <label style={smallLabel}>
                                ...
                                <textarea
                                  ...
                                  value={t.modelAnswer}
                                  onChange={(e) =>
                                    updateTask(d, idx, "modelAnswer", e.target.value)
                                  }
                                  style={{ ...inputStyle, fontFamily: "inherit" }}
                                />
                              </label>
                      ...
                      <div style={{ display: "flex", gap: "10px", marginTop: "6px" }}>
                        ...
                        >
                          {saving ? (
                            ...
                            <>
                              <Save size={15} style={{ marginRight: "6px" }} /> Simpan Dataset
                            </>
                          )}
                        </button>
